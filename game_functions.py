...
def check_event(ship, bullets, screen, ai_settings, button, current_state, aliens, file_operator, ships):
    """监视此时的事件"""
    # 每当用户按键时，都在pygame中注册一个事件。事件都是通过方法pygame.event.get()获取的
    for event in pygame.event.get():
        # 关闭事件
        if event.type == pygame.QUIT:
            sys_exit(current_state, file_operator)
        # 键盘按压事件
        elif event.type == pygame.KEYDOWN:
            key_down_event(event, ship, bullets, ai_settings, screen, current_state, file_operator)
        # 键盘抬起事件
    ...
    check_current_state(ai_settings, bullets, aliens, screen, ship, current_state, ships)


def update_screen(ai_settings, ship, screen, bullets, aliens, current_state,
                  button_one, score_board, highest_board, current_level, ships):
    """更新屏幕"""
    # 填充背景颜色
    ...
    # 绘制最高分对象
    highest_board.draw_board(current_state.highest_score)
    # 绘制此时等级显示对象
    current_level.draw_board(current_state.current_level)
    # 绘制飞船
    ship.blitme()
    # 绘制飞船组
...
def key_down_event(event, ship, bullets, ai_setting, screen, current_state, file_operator):
    """键盘按压事件"""
...
def sys_exit(current_state, file_operator):
    """关闭程序,写入此时的最高得分"""
    file_operator.write_text(str(current_state.highest_score))
    sys.exit()


def check_current_state(ai_setting, bullets, aliens, screen, ship, current_state, ships):
    """查看当前的状态，如果外星人都被消灭则升级难度"""
    # 更新实时状态
    current_state.update_state()
    Ship.remove_ship(current_state.ship_left_number, ships)
    if len(aliens) == 0:
        # 此时外星舰队全部消灭干净,则升级速度
        ...
        bullets.empty()
        ships.clear()
        ship.init_ships_group(ships, screen, ai_setting)
        ship.init_pos()
        Alien.init_aliens_group(aliens, ai_setting, screen)
    else:
        pass
